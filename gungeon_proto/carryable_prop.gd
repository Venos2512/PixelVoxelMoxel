...
var throw_speed: float = 440.0
var throw_drag: float = 360.0

var throw_timer: float = 0.0
var max_throw_time: float = 0.68
...
func _is_nearest_carryable(
	...
	for object_value in get_tree().get_nodes_in_group(
		...
		var other_carried_value = object_node.get(
			"is_carried"
		)

		if (
			other_carried_value != null
			and bool(other_carried_value)
		):
			continue

		var other_thrown_value = object_node.get(
			"is_thrown"
		)

		if (
			other_thrown_value != null
			and bool(other_thrown_value)
		):
			continue
...
func _check_world_impact(
	...
	for blocker_value in get_tree().get_nodes_in_group(
		...
		if blocker.has_method(
			...
			var blocker_radius_value = blocker.get(
				"hit_radius"
			)

			if blocker_radius_value != null:
				var blocker_radius: float = float(
					blocker_radius_value
				)
